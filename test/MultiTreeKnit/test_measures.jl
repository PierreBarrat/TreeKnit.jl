...
@testset "accuracy consistency_rate, output TreeKnit" begin
    ...
    cfull = TreeKnit.consistency_rate(MTK.iter_pairs(MCC_dict)[2]..., input_trees)
    @test c == 0
    @test c == sum(cfull)/3
    MCC12 = [["A"], ["B", "C"]]
    MCC13 = [["A", "B", "C"]]
...
@testset "accuracy_shared_branches" begin
    ...
    true_positive, false_positive, false_negative, true_negative = TreeKnit.accuracy_shared_branches(t2, t1, inferedMCC, realMCC)
