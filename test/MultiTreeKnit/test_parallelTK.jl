...
@testset "3 trees, 1 round" begin
    ...
    MCCs = TreeKnit.parallelized_compute_mccs!(trees, rounds=1)
...
trees = [copy(t) for t in pre_trees]
@btime MCCs = TreeKnit.parallelized_compute_mccs!(trees, rounds=1)
...
@testset "3 trees, 2 rounds" begin
    trees = [copy(t) for t in pre_trees]
    MCCs = TreeKnit.parallelized_compute_mccs!(trees, rounds=2)
    @test SplitList(trees[3]) == SplitList(trees[1])
    @test get(MCCs, (1, 2)) == get(MCCs, (3, 2)) 
...
trees = [copy(t) for t in pre_trees]
@btime MCCs = TreeKnit.parallelized_compute_mccs!(trees, rounds=2)
