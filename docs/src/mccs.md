...
*TreeKnit* reconstructs the ARG from trees by first inferring shared regions of the tree, so called Maximally Compatible Clades (MCC). These regions of the two trees must share the same topology, per default if they share the same topology up to [resolution](@ref resolving) TreeKnit will assume the region is shared. 
