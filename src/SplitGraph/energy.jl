...
function sa_opt(g::Graph; Trange=reverse(1e-3:1e-2:0.6), γ=2., M=10, rep=1, resolve=true, mask=Set(), constraint_cost=γ)
	...
	nfound = 0
	for r in 1:rep
		oconf_, F_ = _sa_opt(g, γ, Trange, M, mask=mask, constraint_cost=constraint_cost)
		Fm = minimum(F_)
		if Fm == Fmin
			append!(oconf, oconf_)
			append!(F, F_)
...
function _sa_opt(g::Graph, γ, Trange, M; mask=Set(), constraint_cost= γ)
	...
	for T in Trange
		if rand() < reset_chance
			tmp_oconf, conf, fmin = doMCMC(g, oconf[rand(1:length(oconf))], M, T=T, γ=γ, mask=mask, constraint_cost=constraint_cost)
		else
			tmp_oconf, conf, fmin = doMCMC(g, conf, M, T=T,γ=γ, mask=mask, constraint_cost=constraint_cost)
		end
		append!(F,fmin)
